...
def send_project_alert(
    ...
    update_note: str = "",
    status: str = "Đã xuất bản (Published)",
    sources: list = None
):
    """Gửi tin nhắn thông báo Dự Án mới hoặc Dự Án cập nhật về Telegram bot."""
    if not TELEGRAM_BOT_TOKEN or not TELEGRAM_CHAT_ID:
        return
        
    is_live = "draft" not in status.lower() and "nháp" not in status.lower()
    studio_link = f"{SITE_BASE_URL}/admin/structure/project;{doc_id}"
    live_link = f"{SITE_BASE_URL}/du-an/{slug}"
    
    if is_update:
        header = "🔄 *DỰ ÁN BĐS ĐÃ ĐƯỢC CẬP NHẬT (SMART MERGE)*"
    else:
        header = "🏢 *DỰ ÁN BĐS MỚI ĐÃ ĐƯỢC TẠO HOÀN CHỈNH*"
        
    links_section = (
        f"🌐 [Xem trực tiếp dự án trên Web]({live_link})\n"
        f"⚙️ [Quản lý trong Sanity Studio]({studio_link})"
    ) if is_live else f"👉 [Bấm vào đây để duyệt và xuất bản trên Sanity Studio]({studio_link})"
    
    sources_text = ""
    if sources:
        sources_text = "\n🔗 *Nguồn tham khảo:*\n" + "\n".join([f"  • {s}" for s in sources[:3]])
        
    update_text = f"\n📝 *Nội dung cập nhật:* {update_note}\n" if update_note else "\n"
    ...
    try:
        resp = requests.post(url, json=payload, timeout=10)
        if resp.status_code == 200:
            print("   📲 Đã gửi thông báo Telegram dự án thành công!")
        else:
            print(f"   [WARN] Gửi Telegram lỗi ({resp.status_code}): {resp.text}")
    except Exception as e:
        print(f"   [WARN] Không thể gửi thông báo Telegram: {e}")
