...
export default async function StaticPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const portableTextComponents = {
    ...
    block: {
      ...
      normal: ({ children }: any) => <p className="portable-text-p" style={{ lineHeight: '1.8', marginBottom: '1rem', color: 'var(--color-text)' }}>{children}</p>,
    ...
    list: {
      bullet: ({ children }: any) => <ul style={{ listStyleType: 'disc', paddingLeft: '1.4rem', marginBottom: '1.2rem', lineHeight: '1.8', color: 'var(--color-text)' }}>{children}</ul>,
      number: ({ children }: any) => <ol style={{ listStyleType: 'decimal', paddingLeft: '1.4rem', marginBottom: '1.2rem', lineHeight: '1.8', color: 'var(--color-text)' }}>{children}</ol>,
    },
    listItem: {
